---
name: security-audit
description: Use when reviewing a change, PR, or repo for application security regressions before committing or merging — especially after touching auth, cookies, CORS, OIDC, endpoints, secrets/.env, dependencies, or reverse-proxy/compose. Built for BFF cookie-session apps (.NET + TanStack + Keycloak behind a proxy) but the checks generalize. For the Claude Code agent config itself (settings.json permissions/sandbox/hooks), use agent-setup-audit instead.
---
...
(`.claude/settings.json` — Bash permissions, sandbox, hooks), use **agent-setup-audit**.
