...
- [x] 1.1 (TDD) Create `plugins/goodreads/goodreads.api`; `IGoodreadsClient` + a `Book` model
...
- [x] 1.2 Implement `GoodreadsPlugin : IPlugin` (id `goodreads`; nav "Reading", `routeBase`
...
- [x] 1.3 (TDD) `CoreApi.Tests` integration tests (fake `IGoodreadsClient`): returns books; `502` on
      failure; requires auth; disabled plugin absent from `/api/plugins`.

## 2. Contracts — shared type + client (TDD)

- [x] 2.1 (TDD) `@pcc/contracts`: `Book` type + `getGoodreads()` client method; client tests (mock fetch).

## 3. Web — read path (SSR-with-data)

- [x] 3.1 (TDD) `lib/server`: `loadGoodreads` + `getGoodreads` server fn; loader unit test (URL).
- [x] 3.2 `goodreads-reading` tile — presentational (`{ books?, error? }`): current shelf titles/count,
      degraded on error; component test. A `BookList` presentational component (cover + title + author)
      + test.
- [x] 3.3 `_authenticated/goodreads` route: loader (`settle(getGoodreads())`) renders the books
...
- [x] 4.3 E2E (Playwright, live stack; configure a public Goodreads user/shelf): login; `/goodreads`
      server-rendered with books; the tile shows the shelf; browser only hit `app.`; `api.` stays `404`.
- [x] 4.4 Update `CLAUDE.md` (the `goodreads` plugin + `Plugins:Goodreads` + the API-is-dead/RSS
