...
- [x] 1.0 **Prereq:** the `base-infra` change provides the GPU-enabled `ollama` service
      (`ollama:11434`). Apply/confirm it first; this change does NOT define the Ollama service.
- [x] 1.1 Add an **nvidia GPU exporter** sidecar to `docker-compose.yml` (nvidia-smi-wrapping image,
      ...
      used/total).
- [x] 1.2 Wire `core-api` env: `Plugins__Models__Enabled`, `Plugins__Models__Ollama__BaseUrl`
...
- [x] 2.1 (TDD) Create `plugins/models/models.api`; `IModelsClient` + the `ModelsStatus` model
...
- [x] 2.2 Implement `ModelsPlugin : IPlugin` (id `models`; nav "Models", `routeBase` `/models`,
...
- [x] 2.3 (TDD) `CoreApi.Tests` integration tests (fake `IModelsClient`): returns models + GPU
...
- [x] 3.1 (TDD) `@pcc/contracts`: `ModelsStatus` (+ nested `InstalledModel`/`RunningModel`/`GpuStat`)
      type + `getModels()` client method; client test (mock fetch); export from `index.ts`.

## 4. Web — read path (SSR-with-data)

- [x] 4.1 (TDD) `lib/server`: `loadModels` + `getModels` server fn; loader unit test (URL).
- [x] 4.2 `models-status` tile — presentational (`{ status?, error? }`): model count + GPU
      util/temp summary, degraded on error; component test.
- [x] 4.3 A `/models` page (`_authenticated/models`): loader (`settle(getModels())`) renders the
...
- [x] 4.4 Wire the `models-status` tile into the dashboard (`_authenticated/index.tsx` loader array +
      destructure + return object + widget branch).

## 5. Verify + done gate

- [x] 5.1 FE gates green: `generate-routes`; `nx run-many -t typecheck lint test build` + prettier.
- [x] 5.2 .NET gates green: `dotnet build` + `dotnet test` + `dotnet format --verify-no-changes`.
- [x] 5.3 E2E (Playwright, live stack; `docker compose up -d --build` with the new `ollama` +
...
- [x] 5.4 Update `CLAUDE.md` (the `models` plugin + `Plugins:Models` + the self-hosted Ollama/GPU
