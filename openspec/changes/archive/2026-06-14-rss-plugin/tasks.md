...
- [x] 1.1 (TDD) Create `plugins/rss/rss.api`; `IFeedClient` + an `RssItem` model
...
- [x] 1.2 Implement `RssPlugin : IPlugin` (id `rss`; nav "Feeds", `routeBase` `/rss`, widget
...
- [x] 1.3 (TDD) `CoreApi.Tests` integration tests (fake `IFeedClient`): aggregated items; `502` when
      empty/failed; requires auth; disabled plugin absent from `/api/plugins`.

## 2. Contracts — shared type + client (TDD)

- [x] 2.1 (TDD) `@pcc/contracts`: `RssItem` type + `getRss()` client method; client tests (mock fetch).

## 3. Web — read path (SSR-with-data)

- [x] 3.1 (TDD) `lib/server`: `loadRss` + `getRss` server fn; loader unit test (URL).
- [x] 3.2 `rss-latest` tile — presentational (`{ items?, error? }`): latest headline or count,
      degraded on error; component test. An `RssItemList` presentational component + test.
- [x] 3.3 `_authenticated/rss` route: loader (`settle(getRss())`) renders the items **server-side**;
...
- [x] 4.3 E2E (Playwright, live stack; configure a known stable feed): login; `/rss` server-rendered
      with items; the `rss-latest` tile shows a headline; browser only hit `app.`; `api.` stays `404`.
- [x] 4.4 Update `CLAUDE.md` (the `rss` plugin + `Plugins:Rss`); mark complete; ready for
      `/opsx:archive`.
