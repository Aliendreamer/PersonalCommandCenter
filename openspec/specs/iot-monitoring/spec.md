# iot-monitoring Specification

## Purpose

The `iot` plugin provides read-only monitoring of Home Assistant entities: it polls Home
Assistant's REST API with a configured base URL and bearer token, exposes the mapped,
domain-filtered entities at `GET /api/iot/entities`, and contributes a "Devices" nav entry,
a `/devices` page, and a dashboard summary tile — degrading gracefully when Home Assistant
is unavailable.
