...
stable `Id`, a `Manifest` (nav label, widget ids, route base), a `Configure` method for
registering its services and options, and a `MapEndpoints` method for registering its HTTP
routes.

#### Scenario: A plugin implements the contract

- **WHEN** a plugin type implementing `IPlugin` is present in the build
- **THEN** the host can read its `Id` and `Manifest` and invoke `Configure` and
  `MapEndpoints` without knowing the concrete type

### Requirement: appsettings-driven activation

The core API SHALL activate a plugin only when `Plugins:{id}:Enabled` is `true` in
configuration, and SHALL pass each activated plugin its own configuration section.

#### Scenario: Enabled plugin is activated

- **WHEN** the core starts with `Plugins:System:Enabled = true`
- **THEN** the `system` plugin's `Configure` and `MapEndpoints` run and its endpoints are
  reachable
